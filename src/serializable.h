...
#define DBGUTIL_SERIALIZE_INT8(os, value)     \
    {                                         \
        DbgUtilErr rcLocal = os.write(value); \
...
#define DBGUTIL_SERIALIZE_INT16(os, value)                                            \
    {                                                                                 \
        DbgUtilErr rcLocal = os.write(os.requiresBigEndian() ? htons(value) : value); \
        if (rcLocal != DBGUTIL_ERR_OK) {                                              \
            return rcLocal;                                                           \
        }                                                                             \
    }
...
#define DBGUTIL_SERIALIZE_INT32(os, value)                                            \
    {                                                                                 \
        DbgUtilErr rcLocal = os.write(os.requiresBigEndian() ? htonl(value) : value); \
        if (rcLocal != DBGUTIL_ERR_OK) {                                              \
            return rcLocal;                                                           \
        }                                                                             \
    }
...
#define DBGUTIL_SERIALIZE_INT64(os, value)                                             \
    {                                                                                  \
        DbgUtilErr rcLocal = os.write(os.requiresBigEndian() ? htonll(value) : value); \
        if (rcLocal != DBGUTIL_ERR_OK) {                                               \
            return rcLocal;                                                            \
        }                                                                              \
    }
...
#define DBGUTIL_SERIALIZE_BOOL(os, value)      \
    {                                          \
        ...
        DbgUtilErr rcLocal = os.write(value8); \
        if (rcLocal != DBGUTIL_ERR_OK) {       \
            return rcLocal;                    \
        }                                      \
    }
...
#define DBGUTIL_SERIALIZE(os, value)                \
    {                                               \
        DbgUtilErr rcLocal = (value).serialize(os); \
        if (rcLocal != DBGUTIL_ERR_OK) {            \
            return rcLocal;                         \
        }                                           \
    }
...
#define DBGUTIL_SERIALIZE_STRING(os, value)                                \
    {                                                                      \
        ...
        DbgUtilErr rcLocal = os.writeBytes(value.c_str(), value.length()); \
        if (rcLocal != DBGUTIL_ERR_OK) {                                   \
            return rcLocal;                                                \
        }                                                                  \
    }
...
#define DBGUTIL_SERIALIZE_NT_STRING(os, value, len)     \
    {                                                   \
        DbgUtilErr rcLocal = os.writeBytes(value, len); \
        if (rcLocal != DBGUTIL_ERR_OK) {                \
            return rcLocal;                             \
        }                                               \
...
#define DBGUTIL_DESERIALIZE_INT8(is, value)  \
    {                                        \
        DbgUtilErr rcLocal = is.read(value); \
        if (rcLocal != DBGUTIL_ERR_OK) {     \
            return rcLocal;                  \
        }                                    \
    }
...
#define DBGUTIL_DESERIALIZE_BOOL(is, value)   \
    {                                         \
        uint8_t value8 = 0;                   \
        DbgUtilErr rcLocal = is.read(value8); \
        if (rcLocal != DBGUTIL_ERR_OK) {      \
            return rcLocal;                   \
        }                                     \
        value = (value8 != 0);                \
...
#define DBGUTIL_DESERIALIZE(is, value)                \
    {                                                 \
        DbgUtilErr rcLocal = (value).deserialize(is); \
        if (rcLocal != DBGUTIL_ERR_OK) {              \
            return rcLocal;                           \
        }                                             \
    }
...
#define DBGUTIL_DESERIALIZE_STRING(is, value)                          \
    {                                                                  \
        ...
        DbgUtilErr rcLocal = is.readBytes(&buf[0], length, bytesRead); \
        if (rcLocal != DBGUTIL_ERR_OK) {                               \
            return rcLocal;                                            \
        }                                                              \
...
#define DBGUTIL_DESERIALIZE_NT_STRING(is, value)                   \
    {                                                              \
        DbgUtilErr rcLocal = is.readUntil([&value](uint8_t byte) { \
            if (byte == 0) {                                       \
                return false;                                      \
            }                                                      \
            value += (char)byte;                                   \
